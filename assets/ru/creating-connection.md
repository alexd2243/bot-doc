...
В роботе поддерживается только подключение к секции Spot биржи HitBTC на основе Websocket и REST API. Маркетдата подключение активируется как описано в главе [Настройка подключений](getting-started.md#connection-setup). Параметры транзакционного подключения описаны ниже.
...
- <details>
    <summary><i>Как сгенерировать пару ключей Ed25519 для отправки API-запросов на Binance<Anchor :ids="['creating-connection.api-key']" /></i></summary>
    Перейдите на официальный сайт генератора асимметричных ключей, чтобы загрузить и установить последнюю версию генератора: https://github.com/binance/asymmetric-key-generator/releases 
      
   ![Alt text](@images/binance_1.png)

    Запустите приложение, и вы сможете использовать его для выбора типа ключа для генерации. Нужно выбрать ``Ed25519``.

    ![Alt text](@images/binance_2.png)

    Создайте новые пары закрытого и открытого ключей ``Ed25519``. Вы также можете вставить существующий закрытый ключ ``Ed25519`` в текстовое поле, и приложение автоматически сгенерирует соответствующий открытый ключ ``Ed25519``.

    Скопируйте ключи или сохраните ключи ``Ed25519`` в файлы ``Private_key.txt`` и ``Public_key.txt``. Обратите внимание, что расширение файла по умолчанию — ``.txt``, но вы можете изменить его на другой формат. Не передавайте свои приватные ключи никому.

    Затем необходимо зарегистрировать ключ на Binance.

    Войдите в свой аккаунт Binance и перейдите в ``Профиль`` - ``Управление API``. Нажмите ``Создать API``.

    ![Alt text](@images/binance_3.png)

    Выберите ``Сгенерированный самостоятельно`` и нажмите ``Далее``.

    ![Alt text](@images/binance_4.png)

    Скопируйте открытый ключ ``Ed25519``, сгенерированный с помощью генератора асимметричных ключей, и вставьте его в поле для регистрации.

     ![Alt text](@images/binance_5.png)

    Введите имя для вашего ключа API. Затем нажмите ``Далее`` и пройдите двухфакторную аутентификацию.

    ![Alt text](@images/binance_6.png)

    В качестве пароля на Viking используете серединную часть закрытого ключа, private key. Например:
    ...
    -----END PRIVATE KEY-----
    ```
    То что использовать в качестве пароля:
    ``MC6BNQAlBQYDK2VwBCIEIEDygO0EhhUR3kn+Dlwe5aZFxIkLaNDZggxey2bszX/y``
...
В связи с особенностями данного подключения полная настройка возможна только после обращения в техническую поддержку.
